...
<div class="bg-white rounded-lg border border-slate-200 p-4 mb-6 text-sm grid grid-cols-2 gap-2 max-w-lg">
  <span class="text-slate-500">Status</span><span>{sale.status}</span>
  <span class="text-slate-500">Profit</span>
  <span class={sale.profit < 0 ? 'text-red-600' : 'text-emerald-600'}>{money(sale.profit)}</span>
</div>

<form method="POST" action="?/update" class="max-w-lg bg-white rounded-lg border border-slate-200 p-6 space-y-4">
  <label class="block">
