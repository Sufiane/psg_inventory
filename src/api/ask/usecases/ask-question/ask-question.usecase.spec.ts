...
describe('AskQuestionUsecase', () => {
    ...
    beforeEach(async () => {
        ...
        const module = await Test.createTestingModule({
            providers: [
                AskQuestionUsecase,
                { provide: IAccountingService, useValue: accounting },
                { provide: IMatchesService, useValue: matches },
                { provide: ILlmService, useValue: llm },
                { provide: RedisService, useValue: redis },
                {
                    provide: ConfigService,
                    useValue: new ConfigService({ ASK_RATE_LIMIT_PER_HOUR: 20 }),
                },
            ],
        }).compile();

        usecase = module.get(AskQuestionUsecase);
    ...
    describe('when the question is within the rate limit', () => {
        it('returns the model answer', async () => {
            const result = await usecase.execute(USER_ID, 'How is the season going?');
        ...
        it('scopes every accounting call to the requesting user', async () => {
            await usecase.execute(USER_ID, 'How is the season going?');
    ...
    describe('when the user has exceeded the hourly rate limit', () => {
        ...
        it('throws a rate limited domain exception', async () => {
            await expect(usecase.execute(USER_ID, 'anything')).rejects.toThrow(
        ...
        beforeEach(async () => {
            const module = await Test.createTestingModule({
                providers: [
                    ...
                    {
                        provide: ConfigService,
                        useValue: new ConfigService({ ASK_RATE_LIMIT_PER_HOUR: 2 }),
                    },
                ],
            }).compile();
    ...
    describe('the profit figures', () => {
        describe('when the period has realized sales', () => {
            it('reports net profit, not gross totalProfit', async () => {
                const result = await usecase.execute(USER_ID, 'anything');
    ...
    describe('when fetching matches for the context', () => {
        ...
        it('fetches matches for the same season year it fetches amortization for', async () => {
            await usecase.execute(USER_ID, 'anything');
